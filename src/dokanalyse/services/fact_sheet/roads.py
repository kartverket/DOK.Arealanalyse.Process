...
async def _get_data(geometry: ogr.Geometry, epsg: int) -> List[Dict[str, Any]]:
    start = time.time()
    status, response = query_ogc_api(_API_BASE_URL, _LAYER_NAME, geometry, None, epsg)
    end = time.time()
    ...
    _logger.info('Fact sheet: Got roads from Elveg OGC API',
                 duration=round(end - start, 2))

    return _map_response(response)


def _map_response(response: Dict[str, Any]) -> List[Dict[str, Any]]:
